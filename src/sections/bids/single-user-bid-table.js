...
export const SingleUserBidTable = (props) => {
    const {
        // count = 0,
        // items = [],
        valuesResult,
    ...
    const filteredItems = sortedDate?.filter((customer) =>
        ...
        customer.game_type.toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    const fetchData = async () => {
        try {
            ...
            const q = query(collection(db, 'User_Events'), where('phone', '==', valuesResult.phone));
            await onSnapshot(q, (querySnapshot) => {
                if (querySnapshot.empty) {
                    ...
                    setResultData(querySnapshot.docs.map(doc => ({
                        ...
                        points: doc.data().points ? doc.data().points : 'N/A',
                    })));
