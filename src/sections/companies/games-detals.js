...
export const GamesDetails = ({ game }) => {
    ...
    function mapSinglePannaToGroups(singlePanna, groupSize) {
        const groups = singlePanna.reduce((acc, value, index) => {
            const groupIndex = Math.floor(index / groupSize);
            if (!acc[groupIndex]) {
                acc[groupIndex] = [];
            }
            acc[groupIndex].push(value.label);
            return acc;
        }, []);
    ...
    const groupedData = mapSinglePannaToGroups(singlePanna, 12);
